...
function deriveOverallStatus(checks: Record<string, ComponentCheckResult>): CheckState {
...
async function checkSentry(): Promise<ComponentCheckResult> {
  ...
  try {
    const diagnosis = await withTimeout(diagnoseSentryConnectivity(), DEFAULT_TIMEOUT_MS, 'sentry connectivity')
...
async function diagnoseSentryConnectivity(): Promise<'no-client-active' | 'no-dsn-configured' | void> {
